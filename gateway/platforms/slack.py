...
def _build_clck_slack_client_context(channel_id: str) -> Optional[_CLCKSlackClientContext]:
    ...
    The lookup is intentionally local-only and fail-open: missing/invalid files
    or unmatched channels simply return None so normal Slack handling continues.
    """
...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        is_mentioned = bool(bot_uid and mention_token in routing_text)
        event_thread_ts = event.get("thread_ts")
        is_thread_reply = bool(event_thread_ts and event_thread_ts != ts)
        free_response_channel = channel_id in self._slack_free_response_channels()

        # Derive a clean command candidate from Slack's plain text before
        # channel gating.  Active in-memory sessions may need /stop, /new, or
        # /reset to unblock a thread even after allowed_channels has changed;
        # normal messages and non-session commands still obey the whitelist.
        if is_mentioned and mention_token:
        ...
        active_session_control_command = False
        if is_thread_reply and is_slack_gateway_command:
            cmd_name = slack_command_text[1:].split(maxsplit=1)[0].split("@", 1)[0].lower()
            if cmd_name in {"stop", "new", "reset"}:
                try:
                    ...
                    active_session_control_command = active_key in self._active_sessions
                except Exception:  # pragma: no cover - defensive
                    active_session_control_command = False

        if not is_dm and bot_uid:
            # Check allowed channels first: whitelisted Slack bots must not be
            # bypassed by free-response channels or thread/session continuation.
            # Exception: reset-like controls for an already-active thread are
            # allowed so operators can stop/clear stale work after a whitelist
            # change without waiting for a gateway restart.
            allowed_channels = self._slack_allowed_channels()
            if (
                allowed_channels
                and channel_id not in allowed_channels
                and not active_session_control_command
            ):
