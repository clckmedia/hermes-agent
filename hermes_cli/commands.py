...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("state", "Show read-only current session state report", "Info",
               gateway_only=True, aliases=("state-report",)),
...
# Backwards-compatible CLCK name used by the curated manifest code.
_SLACK_RESERVED_COMMAND_NAMES = _SLACK_RESERVED_COMMANDS
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    A curated subset of gateway-available commands in ``COMMAND_REGISTRY`` is
    surfaced as standalone Slack slash commands (e.g. ``/btw``, ``/stop``,
    ``/model``), with ``/hermes <verb>`` kept as the fallback for everything
    outside Slack's practical native command cap.

    Canonical names, aliases, and plugin-registered commands may be included
    when they fit the curated/reserved-name rules below.

    Commands whose sanitized name collides with a Slack built-in or CLCK
    reserved/excluded native command (e.g. ``/status``, ``/help``, ``/me``,
    ``/join``) are skipped from the native manifest. Users can still reach
    them via ``/hermes <command>``.
    ...
    as the catch-all first entry so commands outside the native picker still
    work as ``/hermes <subcommand>`` or free-form questions.
    """
    overrides = _resolve_config_gates()
    entries: list[tuple[str, str, str]] = []
    seen: set[str] = set()
