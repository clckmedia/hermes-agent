...
@pytest.mark.asyncio
async def test_session_hygiene_informs_user_when_codex_local_timeout_digest_used(monkeypatch, tmp_path):
    """Gateway hygiene should report Codex local-digest recovery without config advice."""
    fake_dotenv = types.ModuleType("dotenv")
    fake_dotenv.load_dotenv = lambda *args, **kwargs: None
    monkeypatch.setitem(sys.modules, "dotenv", fake_dotenv)

    class FakeCompressAgentWithLocalDigest:
        last_instance = None
        ...
        def __init__(self, **kwargs):
            ...
            self.context_compressor = SimpleNamespace(
                ...
                _last_summary_dropped_count=0,
                _last_summary_error=None,
                _last_local_timeout_digest_used=True,
                _last_local_timeout_digest_error=(
        ...
        def _compress_context(self, messages, *_args, **_kwargs):
            self.session_id = f"{self.session_id}_compressed"
            return ([{"role": "assistant", "content": "local digest summary"}], None)

    fake_run_agent = types.ModuleType("run_agent")
    fake_run_agent.AIAgent = FakeCompressAgentWithLocalDigest
    monkeypatch.setitem(sys.modules, "run_agent", fake_run_agent)
    ...
    local_notes = [
        s for s in adapter.sent
        if "Codex compression summariser timed out after 120s" in s["content"]
    ]
    assert len(local_notes) == 1, (
        f"Expected 1 local-digest notice, got {len(local_notes)}: {adapter.sent}"
    )
    note = local_notes[0]
    assert "local continuity digest" in note["content"]
    assert "context is intact" in note["content"]
    assert "No config change needed" in note["content"]
    assert "Configured compression model" not in note["content"]
    assert "Recovered using" not in note["content"]
    assert "auxiliary.compression.model" not in note["content"]
    assert note["chat_id"] == "-1001"
    assert note["metadata"] == {"thread_id": "17585"}

    FakeCompressAgentWithLocalDigest.last_instance.close.assert_called_once()
