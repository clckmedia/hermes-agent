...
def test_allowed_channels_blocks_free_response_non_whitelisted_channel():
    """Free-response channels cannot bypass the allowed_channels whitelist."""
    adapter = _make_adapter(
        allowed_channels=[CHANNEL_ID],
        free_response_channels=[OTHER_CHANNEL_ID],
    )
    assert _would_process(adapter, channel_id=OTHER_CHANNEL_ID, text="hello") is False


def test_allowed_channels_blocks_thread_continuation_non_whitelisted_channel():
    """Thread/active-session continuation cannot bypass allowed_channels."""
    adapter = _make_adapter(
        allowed_channels=[CHANNEL_ID],
        free_response_channels=[OTHER_CHANNEL_ID],
    )
