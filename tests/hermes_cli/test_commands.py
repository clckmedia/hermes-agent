...
class TestSlackNativeSlashes:
    ...
    def test_excludes_reserved_slack_commands(self):
        """Reserved/built-in commands stay out of the native manifest.
        ...
        for reserved in _SLACK_RESERVED_COMMANDS:
            assert reserved not in names, (
                f"/{reserved} is reserved and must not appear in the native manifest"
            )
    ...
    def test_non_native_commands_still_route_through_hermes_subcommands(self):
        ...
        assert "new" not in native_names
