...
def _get_lock_paths() -> tuple[Path, Path]:
    ...
    if _LOCK_DIR is not None or _LOCK_FILE is not None:
        lock_file = Path(_LOCK_FILE) if _LOCK_FILE is not None else Path(_LOCK_DIR) / ".tick.lock"
        lock_dir = Path(_LOCK_DIR) if _LOCK_DIR is not None else lock_file.parent
