...
def _handle_send(args):
    ...
    # Accept real platform names dynamically: built-ins resolve to enum members,
    # and known plugin platforms create dynamic members via Platform._missing_().
    # Arbitrary strings are still rejected by gateway.config.Platform.
    try:
        platform = Platform(platform_name)
    except (ValueError, KeyError):
        available = ", ".join(sorted(p.value for p in Platform))
        return tool_error(f"Unknown platform: {platform_name}. Available: {available}")
...
async def _send_zulip(pconfig, chat_id, message, thread_id=None):
    ...
    try:
        ...
        result = await adapter.send(chat_id, message, metadata={"thread_id": thread_id} if thread_id else None)
        if not result.success:
