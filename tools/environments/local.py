...
class LocalEnvironment(BaseEnvironment):
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        # Recover when the requested cwd has been deleted out from under us —
        # usually by a previous tool call that ran ``rm -rf`` on its own
        # working dir (issue #17558).  Popen would otherwise raise
        # FileNotFoundError before bash starts, wedging every subsequent call
        # until the gateway restarts.  Preserve CLCK's explicit popen_cwd
        # support: a caller-supplied cwd still wins over the session cwd, but
        # invalid/deleted paths are handed to Popen as a safe existing parent.
        requested_cwd_raw = popen_cwd or self.cwd
        requested_cwd = (
            os.path.abspath(os.path.expanduser(requested_cwd_raw))
            if requested_cwd_raw
            else ""
        )
        spawn_cwd = _resolve_safe_cwd(requested_cwd)
        session_cwd_request = not popen_cwd or popen_cwd == self.cwd
        if spawn_cwd != requested_cwd:
            if session_cwd_request:
                logger.warning(
                ...
                logger.warning(
                    "LocalEnvironment explicit cwd %r is missing on disk; "
                    "falling back to %r for subprocess spawn.",
                    popen_cwd,
                    spawn_cwd,
                )

        # On Windows, cwd values may be Git Bash-style paths (/c/Users/...).
        # subprocess.Popen needs a native Windows path.
        _popen_cwd = spawn_cwd
    ...
    def _update_cwd(self, result: dict):
        ...
        Skip assignments when the marker path no longer exists as a directory —
