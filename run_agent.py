...
class AIAgent:
    ...
    _MEMORY_REVIEW_PROMPT = (
        ...
        "Do NOT save client/project details to baseline memory; put operational details "
        "in docs, registries, or skills unless they are compact facts that will change "
    ...
    _COMBINED_REVIEW_PROMPT = (
        ...
        "preferences with the memory tool. Do NOT save client/project details "
        "to baseline memory; put operational details in docs, registries, or skills "
        "unless they are compact facts that will change decisions constantly.\n\n"
        ...
        "which compact facts change decisions constantly'; skills say 'how "
        "to do this class of task for this user'. Both "
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        existing_prompt_text = "\n\n".join(
            p.strip()
            for p in [*stable_parts, *context_parts]
            if p and p.strip()
        )
        always_include_skills_prompt = build_always_include_skills_prompt(
            existing_prompt_text=existing_prompt_text,
            task_id=self.session_id,
            available_tools=self.valid_tool_names,
            available_toolsets=avail_toolsets,
        )
        if always_include_skills_prompt:
            context_parts.append(always_include_skills_prompt)
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    if _compressor.last_prompt_tokens > 0:
                        # Only use provider-reported prompt_tokens from usage;
                        # completion/reasoning tokens do not consume the next
                        # prompt context window. Thinking models can inflate
                        # completion_tokens with reasoning, causing premature
                        # compression if those are included. (#12026)
                        _reported_prompt_tokens = _compressor.last_prompt_tokens
                    # Include the current messages, active system prompt, and
                    # tool schemas. Large tool results appended after the last
                    # provider usage sample and broad tool schemas can push the
                    # next request past the threshold even when prompt_tokens
                    # alone looks safe. (#14695)
